...
if( ! function_exists('colorist_recent_posts') ) {
	function colorist_recent_posts() {
		...
		if ( $query->have_posts() ) {
			...
			$output .= '<div class="post-wrapper-head"><h2>' . apply_filters('colorist_post_title',__('Recent Posts','colorist') ) . '</h2></div>';
...
/* Admin notice */
/* Activation notice */
add_action( 'load-themes.php',  'colorist_one_activation_admin_notice'  );

if( !function_exists('colorist_one_activation_admin_notice') ) {
	function colorist_one_activation_admin_notice() {
        global $pagenow;
	    if ( is_admin() && ('themes.php' == $pagenow) && isset( $_GET['activated'] ) ) {
	        add_action( 'admin_notices', 'colorist_admin_notice' );
	    } 
	} 
}  
 ...
 * @return bool|null
 */
function colorist_admin_notice() { ?>
    <div class="updated notice notice-alt notice-success is-dismissible">  
        <p><?php printf( __( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our <a href="%2$s">Welcome page</a>', 'colorist' ), 'Colorist', esc_url( admin_url( 'themes.php?page=colorist_upgrade' ) ) ); ?></p>
    	<p><a href="<?php echo esc_url( admin_url( 'themes.php?page=colorist_upgrade' ) ); ?>" class="button" style="text-decoration: none;"><?php _e( 'Get started with Colorist', 'colorist' ); ?></a></p>
    </div><?php
}